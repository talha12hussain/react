...
import Button from '../Logout/Button';
...
const Navbar = () => {
  ...
  return (
    <div>
      <div className=' flex flex-row justify-between p-5 md:px-32 px-5 bg-darkBackground text-white'>
        <div className=' flex items-center'>
          ...
          <h1 className=' font-semibold text-xl cursor-pointer'>Tripixis</h1></Link>
        </div>
        <nav className=' hidden lg:flex  flex-row items-center gap-6'>
          ...
          <Link to="/" spy={true} smooth={true} duration={500} className='  hover:text-brightColor transition-all  cursor-pointer'>Feature</Link>
          <Link to="/" spy={true} smooth={true} duration={500} className='  hover:text-brightColor transition-all  cursor-pointer'>Destination</Link>
    ...
    <div className={`${menu ? " translate-x-0 " : "-translate-x-full"} lg:hidden flex flex-col absolute bg-darkBackground text-white left-0 top-20 font-semibold text-2xl text-center pt-8 pb-4 gap-4 w-full h-fit transition duration-300`}> 
    <Link to="/" spy={true} smooth={true} duration={500} className=' hover:text-brightColor transition-all cursor-pointer'>Home</Link>
          <Link to="/" spy={true} smooth={true} duration={500} className='  hover:text-brightColor transition-all  cursor-pointer'>Feature</Link>
          <Link to="/" spy={true} smooth={true} duration={500} className='  hover:text-brightColor transition-all  cursor-pointer'>Destination</Link>
          <Link to="/" spy={true} smooth={true} duration={500} className='  hover:text-brightColor transition-all  cursor-pointer'>About</Link>
